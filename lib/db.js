...
function userDreams(uid) {
  return collection(firestore, 'users', uid, 'dreams');
}
function userSettingsRef(uid) {
  return doc(firestore, 'users', uid, 'meta', 'settings');
}
...
export async function saveDream(session, dream) {
  ...
  const ref = doc(firestore, 'users', session.uid, 'dreams', id);
...
export async function deleteDream(session, id) {
  ...
  await deleteDoc(doc(firestore, 'users', session.uid, 'dreams', id));
}
